...
// -------------------- REACTION TIMER --------------------
const unsigned long reactionDuration = 30000;   // 30 seconds
...
String iodineLevel = "Unknown";
float iodineConcentration = -1.0;
...
// void sendResultsToFirebase() {
//   if (!wifiConnected || !firebaseInitialized || !app.ready()) {
...
//   Database.set<bool>(aClient, testPath + "/ageAbove6", ageAbove6, processData, "ageAbove6");

//   Database.set<float>(aClient, testPath + "/blankAverage", blankAverage, processData, "blankAverage");
//   Database.set<float>(aClient, testPath + "/sampleAverage", sampleAverage, processData, "sampleAverage");
//   Database.set<float>(aClient, testPath + "/transmittance", transmittance, processData, "transmittance");
//   Database.set<float>(aClient, testPath + "/absorbance", absorbance, processData, "absorbance");
//   Database.set<float>(aClient, testPath + "/iodineConcentration", iodineConcentration, processData, "iodineConc");
...
void doSampleMeasurement() {
  ...
  // Example placeholder logic for Firebase values
  iodineConcentration = absorbance * 100.0;

  if (iodineConcentration < 20) {
    iodineLevel = "Low";
  } else if (iodineConcentration < 40) {
    iodineLevel = "Medium";
  } else {
    iodineLevel = "High";
  }
...
void setup() {
  ...
  lcd.print("IODetect");
  lcd.setCursor(0, 1);
  if (wifiConnected) lcd.print("WiFi connected");
